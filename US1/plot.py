...
t_l=t_l*10**(-6)
plt.plot(t_l*10**(6), theorie(t_l,*params)*10**3, 'b-', label="Ausgleichsgerade")
...
t_ls=t_ls*10**(-6)
plt.plot(t_ls*10**(6), theorie(t_ls,*params)*10**3, 'b-', label="Ausgleichsgerade")
plt.xlim(11,40)
plt.ylabel(r"$s$/$\si{\milli\meter}")
plt.xlabel(r"$t$/$\si{\micro\second}$")
plt.legend(loc='best')
...
plt.clf()
s,a2,a1=np.genfromtxt("Messdaten/amplitude.txt", unpack=True)
...
plt.plot(s_ls*10**3, theorie(s_ls,*params), 'b-', label="Ausgleichsgerade")
plt.xlim(25,88)
